            ...
            steps {
                script {
                    ...
                    sh """
                        ...
                        jq --arg DOCKER_IMAGE "$DOCKER_IMAGE" 'del(.taskDefinitionArn, .revision, .status, .requiresAttributes, .compatibilities, .registeredAt, .registeredBy) | .containerDefinitions[0].image = $DOCKER_IMAGE' raw_taskdef.json > taskdef.json
